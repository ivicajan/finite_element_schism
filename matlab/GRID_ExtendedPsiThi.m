function [LON_psi2, LAT_psi2, MSK_psi2, DEP_psi2]=GRID_ExtendedPsiThi(...
...
%
DEP_psi2=zeros(eta_psi+2, xi_psi+2);
for iEta=2:eta_psi2-1
  for iXi=2:xi_psi2-1
    DEP_psi2(iEta, iXi)=DEP_psi(iEta-1, iXi-1);
  end;
end;
for iEta=2:eta_psi2-1
  DEP_psi2(iEta, 1)=DEP_rho(iEta-1,1)+DEP_rho(iEta,1)-DEP_psi2(iEta,2);
  DEP_psi2(iEta, xi_psi2)=DEP_rho(iEta-1,xi_rho)+DEP_rho(iEta,xi_psi) ...
      - DEP_psi2(iEta,xi_psi2-1);
end;
for iXi=2:xi_psi2-1
  DEP_psi2(1, iXi)=DEP_rho(1, iXi-1)+DEP_rho(1, iXi) ...
      -DEP_psi2(2,iXi);
  DEP_psi2(eta_psi2,iXi)=DEP_rho(eta_rho,iXi-1)+DEP_rho(eta_rho,iXi) ...
      -DEP_psi2(eta_psi2-1,iXi);
end;
%
DEP_psi2(1,1)=(DEP_psi2(1,2) + DEP_psi2(2,1))/2;
DEP_psi2(eta_psi2,1)=...
    (DEP_psi2(eta_psi2-1,1)+DEP_psi2(eta_psi2,2))/2;
DEP_psi2(eta_psi2,xi_psi2)=...
    (DEP_psi2(eta_psi2-1,xi_psi2)+DEP_psi2(eta_psi2,xi_psi2-1))/2;
DEP_psi2(1,xi_psi2)=...
    (DEP_psi2(1,xi_psi2-1)+DEP_psi2(2,xi_psi2))/2;
